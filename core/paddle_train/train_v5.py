"""PP-OCRv5 학습 시작 (RTX 4070Ti 최적화, venv_ocr 사용) - 최종 버전"""
import os
import sys
import subprocess
import yaml
...
# train_v5.py는 yuzyproject-aimodels/core/paddle_train/에 있음
SCRIPT_DIR = Path(__file__).resolve().parent
# PROJECT_ROOT는 yuzyproject-aimodels (core의 부모)
PROJECT_ROOT = SCRIPT_DIR.parent.parent
VENV_OCR = PROJECT_ROOT / "venv_ocr"
VENV_OCR_PYTHON = VENV_OCR / "Scripts" / "python.exe"

# 출력 디렉토리는 PROJECT_ROOT/output에 생성
OUTPUT_DIR = PROJECT_ROOT / "output"

# PaddleOCR 소스 코드 경로 (기존 설정 참고)
PADDLEOCR_ROOT = Path("C:/Pyg/Tools/PaddleOCR")
if not PADDLEOCR_ROOT.exists():
    # 대체 경로 시도
    PADDLEOCR_ROOT = Path("C:/PaddleOCR")
    if not PADDLEOCR_ROOT.exists():
        PADDLEOCR_ROOT = None
...
REC_CONFIG = CONFIG_DIR / "rec_ke_v5_train.yml"
...
(OUTPUT_BASE / "rec_model").mkdir(exist_ok=True)
(OUTPUT_BASE / "det_model").mkdir(exist_ok=True)
(OUTPUT_BASE / "rec_inference").mkdir(exist_ok=True)
...
print("=" * 70)
print("PP-OCRv5 학습 시작 (RTX 4070Ti 최적화, venv_ocr)")
print("=" * 70)
print(f"\n[스크립트 위치] {SCRIPT_DIR}")
...
if not REC_CONFIG.exists():
    print(f"\n[ERROR] Recognition Config 파일을 찾을 수 없습니다: {REC_CONFIG}")
    sys.exit(1)
...
print(f"\n[Config 파일]")
print(f"  Recognition: {REC_CONFIG}")
...
# Recognition Config 업데이트
with open(REC_CONFIG, 'r', encoding='utf-8') as f:
    rec_config = yaml.safe_load(f)

rec_config['Global']['save_model_dir'] = str(OUTPUT_BASE / "rec_model")
rec_config['Global']['save_inference_dir'] = str(OUTPUT_BASE / "rec_inference")
# character_dict_path는 기존 경로 유지
char_dict_path = Path(rec_config['Global']['character_dict_path'])
if not char_dict_path.exists():
    print(f"  [경고] character_dict_path를 찾을 수 없습니다: {char_dict_path}")
rec_config['PostProcess']['character_dict_path'] = str(char_dict_path)

REC_CONFIG_TEMP = BASE_DIR / "configs" / f"rec_ke_v5_train_{TIMESTAMP}.yml"
with open(REC_CONFIG_TEMP, 'w', encoding='utf-8') as f:
    yaml.dump(rec_config, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
print(f"  Recognition Config 임시 파일: {REC_CONFIG_TEMP}")
...
DET_CONFIG_TEMP = BASE_DIR / "configs" / f"det_ke_v5_train_{TIMESTAMP}.yml"
...
if PADDLEOCR_ROOT and (PADDLEOCR_ROOT / "tools" / "train.py").exists():
    ...
    # PaddleOCR 소스를 찾을 수 없으면 직접 실행 시도
    TRAIN_SCRIPT = None
...
# 학습 시작
print("\n" + "=" * 70)
print("[1/2] Recognition 모델 학습 시작")
print("=" * 70)
print(f"  Config: {REC_CONFIG}")
print(f"  Python: {VENV_OCR_PYTHON}")
print(f"  작업 디렉토리: {TRAIN_WORK_DIR}")
if TRAIN_SCRIPT:
    print(f"  학습 스크립트: {TRAIN_SCRIPT}")
    print(f"  명령어: python tools/train.py -c {REC_CONFIG}")
else:
    print(f"  명령어: python -m paddleocr.tools.train -c {REC_CONFIG}")
print("\n")
...
if rec_result.returncode != 0:
    print("\n[ERROR] Recognition 학습 실패!")
    print(f"  종료 코드: {rec_result.returncode}")
    sys.exit(rec_result.returncode)

print("\n" + "=" * 70)
print("[2/2] Detection 모델 학습 시작")
print("=" * 70)
...
print("=" * 70)
print(f"\n[출력 디렉토리] {OUTPUT_BASE}")
print(f"  - Recognition 모델: {OUTPUT_BASE / 'rec_model'}")
print(f"  - Detection 모델: {OUTPUT_BASE / 'det_model'}")
print(f"  - Recognition 추론: {OUTPUT_BASE / 'rec_inference'}")
print(f"  - Detection 추론: {OUTPUT_BASE / 'det_inference'}")

# 임시 Config 파일 정리
try:
    if 'REC_CONFIG_TEMP' in locals() and REC_CONFIG_TEMP.exists():
        REC_CONFIG_TEMP.unlink()
    if 'DET_CONFIG_TEMP' in locals() and DET_CONFIG_TEMP.exists():
