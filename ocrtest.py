...
def merge_tesseract_winrt_results(
    ...
    """
    Tesseract와 WinRT 결과를 병합
    - 레이아웃: Tesseract WordBox 위치 사용
    - 텍스트: 겹치는 부분은 유지, 한글은 WinRT 우선, 나머지는 Tesseract 우선
    """
    # 한글 감지 정규식
    ...
    def find_overlapping_winrt_word(tess_word: WordBox, winrt_words_list: List[WordBox], threshold: float = 0.5) -> Optional[WordBox]:
        """Tesseract WordBox와 겹치는 WinRT WordBox 찾기"""
        best_match = None
        best_overlap = 0.0
        ...
        for winrt_word in winrt_words_list:
            ...
            winrt_area = winrt_word.w * winrt_word.h
    ...
    # WinRT에만 있는 단어 추가 (Tesseract에서 인식하지 못한 한글 등)
    for winrt_word in winrt_words:
        # Tesseract WordBox와 겹치지 않는 WinRT 단어 찾기
...
def main():
    ...
    try:
        ...
        # 2. 결과 병합: 둘 다 먼저 인식하고, 겹치는 부분은 유지, 나머지는 레이아웃은 Tesseract, 한글은 WinRT
        if tesseract_words and winrt_words:
            try:
                print("  → 결과 병합 중 (레이아웃: Tesseract, 한글: WinRT)...")
